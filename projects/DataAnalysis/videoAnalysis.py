...
class videoAnalysis(object):
        ...
        for x in range(len(self.targetFrameList)):
            if x == len(self.targetFrameList) - 1:
                if not previously_present:
                    self.targetTimesPresent['startTime'].append(self.targetFrameList[x] * (1 / json_data_parsed['videoMetadata']['FPS']))
                    self.targetTimesPresent['endTime'].append(self.targetFrameList[x] * (1 / json_data_parsed['videoMetadata']['FPS']))
                    break
        ...
        for label in self.generalObjectsFoundAnalysisData.keys():
            ...
            for x in range(len(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'])):
                if x == len(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh']) - 1:
                    if not previously_present:
                        self.generalObjectsFoundAnalysisData[label]['timeRange']['startTime'].append(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] * (1 / json_data_parsed['videoMetadata']['FPS']))
                        self.generalObjectsFoundAnalysisData[label]['timeRange']['endTime'].append(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] * (1 / json_data_parsed['videoMetadata']['FPS']))
                        break
                    if previously_present:
                        self.generalObjectsFoundAnalysisData[label]['timeRange']['endTime'].append(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] * (1 / json_data_parsed['videoMetadata']['FPS']))
                        break
                if not previously_present:
                    self.generalObjectsFoundAnalysisData[label]['timeRange']['startTime'].append(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] * (1 / json_data_parsed['videoMetadata']['FPS']))
                if self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] + 1 == self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x+1]:
                    previously_present = True
                    continue
                if self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] + 1 != self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x+1]:
                    self.generalObjectsFoundAnalysisData[label]['timeRange']['endTime'].append(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'][x] * (1 / json_data_parsed['videoMetadata']['FPS']))
    ...
    def calculateAverageGenObjectConfidence(self, table):
        ...
        for label in self.generalObjectsFoundAnalysisData.keys():
            ...
            self.generalObjectsFoundAnalysisData[label]['percentage'] = str((len(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'])/self.totalFrames) * 100) + "%"
            self.generalObjectsFoundAnalysisData[label]['screenTime'] = str(len(self.generalObjectsFoundAnalysisData[label]['framesAboveThresh'])/json_data_parsed['videoMetadata']['FPS']) + " seconds"

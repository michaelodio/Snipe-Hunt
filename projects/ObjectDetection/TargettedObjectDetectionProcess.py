...
class TargettedObjectDetection(object):
    ...
    def __init__(self):
        """ Constructor """
        self.logger = Utilities.setup_logger("targetted-obj",
                                             "../../logs/TargettedObjectDetection.log")  # add verbosity
    ...
    def run(self):
        ...
        for m in consumer:
            ...
            frame = Utilities.decodeFrame(
                json_data_parsed)  # take parsed string and send it to utilities to decode it from base64
            ...
            confidenceStat = labelImage(self.graph, self.labels, self.input_layer, self.output_layer, self.input_height,
                                        self.input_width, frameAsTensor, self.logger)  # tests frame for targeted object
            ...
            Utilities.storeJson(json_data,
                                "../../res/FramesMetadataTargetImgClassification/" + json_data_parsed['videoMetadata'][
                                    'videoName'] + "_Metadata" + str(json_data_parsed['frameMetadata'][
                                                                         'frameNum']) + ".txt")  # Store updated metadata Jsons locally
            Utilities.exportJson(json_data, self.topic_name_out)  # export updated Json files to kafka topic 'target'
        consumer.close()
        self.logger.info("Targetted Object Detection consumer closed")
